...
echo "" >> "$SUMMARY"
echo "| Route | A11y | SEO | Perf | PWA |" >> "$SUMMARY"
echo "|---|---|---|---|---|" >> "$SUMMARY"
...
for path in "${ROUTES[@]}"; do
  ...
  npx --yes lighthouse "$BASE_URL$path" \
    --only-categories=accessibility,seo,performance,pwa \
    --form-factor=mobile \
    --output=json \
    --output-path="$json" \
  ...
  a11y=$(node -e "const r=require('$json');console.log(Math.round(r.categories.accessibility.score*100))")
  ...
  pwa=$(node -e "const r=require('$json');console.log(Math.round((r.categories.pwa?.score??0)*100))")

  echo "| \`$path\` | $a11y | $seo | $perf | $pwa |" >> "$SUMMARY"
  ...
  [ "$pwa"  -ge $THRESH_PWA  ] || { echo "FAIL pwa  on $path: $pwa  < $THRESH_PWA"  >&2; fail=1; }
done

echo "" >> "$SUMMARY"
echo "Thresholds: A11y ≥ $THRESH_A11Y, SEO ≥ $THRESH_SEO, Perf ≥ $THRESH_PERF, PWA ≥ $THRESH_PWA." >> "$SUMMARY"
