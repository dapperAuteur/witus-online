...
// E2E + a11y gate (plan 30 Phase 2). Two ways to point it at a running app:
...
// No webServer block on purpose: this repo's dev server needs real env (NEXTAUTH_SECRET, EMAIL_*,
// database) that CI and fresh clones don't have. CI always runs against a deployed URL instead.
...
export default defineConfig({
  ...
  projects: [
    ...
    {
      // The charter is mobile-first at 360px (plans/00-descriptions.md §3); a flow that only works
      // on desktop is a failing flow. Same specs, small viewport.
      name: "mobile",
      use: {
        ...devices["Pixel 7"],
